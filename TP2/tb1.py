...
ventanas = [np.bartlett(N), wds.triang(N), np.hanning(N), wds.hann(N),    np.blackman(N), wds.blackamanHarris(N), sg.flattop(N), wds.flattop(N)]
V =  len(ventanas)
ventana = ["Bartell", "my_Triang", "Hanning", "my_Hanning", "Blackman", "my_Blackman", "Flattop", "my_Flattop"]
...
mod_signal_padding = np.vstack(np.transpose([np.abs(np.fft.fft(signal_padding[:,ii]))*2/N  for ii in      range(V)]))
...
#grafico del modulo la ventana con zero padding de python
     ...
     f_ax3.plot(freq[0:int(N/2)], mod_signal_padding[0:int(N/2),ii], marker='.', linestyle='None')
...
#grafico del modulo la ventana con zero padding propia
     ...
     f_ax6.plot(freq[0:int(N/2)], mod_signal_padding[0:int(N/2),ii + 1], marker='.', linestyle='None')
